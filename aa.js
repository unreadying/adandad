...
//document.body.addEventListener('click',bodymenu);
...
function banners(){
	if( document.querySelector('.bannerv') ){
	bannervw = document.querySelector('.bannerv').offsetHeight;
		bannerv = document.querySelector('.bannerv') ;
	}
	if (document.querySelector('.banner')){
	bannerhw = document.querySelector('.banner').offsetWidth;
	bannerh = document.querySelector('.banner') ;
	}
	aniloop();
}
...
function expand(event){
	...
	if(id=="service" || id=="serviceex")
		info(id,"service","we are always looking for new clients , people who want to cultivate or grow a successful brand . we bring the essential advertising toolkit to anyone who asks , no matter the budget . we provide : SEO , social media , web design and hosting , unique visual style , and good old fasioned advertisements . <br><br> small business character , big business energy . <br><br> there is something genuine about small businesses ; at first it can be offputting , but once customers give it a chance they will swear by it . on the other hand , big businesses tend to feel manufactured ; they are easily approachable , but customers rarely feel a sense of connection with the brand . here at ad&ad we strive to marry these two approaches , bringing the trustworthy energy of big business & the lovable character of small business to each and every brand we work alongside . <br><br> we want to form genuine connections with our clients , our agency stands against the individualism and anxiety of the modern world . if you are interested in working alongside us ... send an <b><a id='email' href='contact.html'>email</a></b> !");
...
function aniloop(){
			...
			bannerop+=0.1;
					...
					bannerpos-=2;
	if(bannerh && bannerpos<bannerhw ){
		bannercrawl();
	}
	if(bannerv  && bannerpos<bannervw ){
		bannerclimb();
	}
	setTimeout(aniloop,40);
}

function bannercrawl(){

		bannerh.style.left=''+bannerpos+'px'

			bannerh.style.opacity=''+bannerop+''
		
		setTimeout(bannercrawl,5);
	
}

function bannerclimb(){

		bannerv.style.top=''+bannerpos+'px'

			bannerv.style.opacity=''+bannerop+''
	
		setTimeout(bannerclimb,5);
